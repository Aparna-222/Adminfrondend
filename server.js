...
app.post('/departments', (req, res) => {
    ...
    db.execute(query, [name, description], (err, results) => {
        ...
        res.status(201).json({ message: 'Department added', departmentId: results.insertId });
    });
});
...
app.post('/sections', (req, res) => {
    ...
    const query = 'INSERT INTO sections (department, section, description) VALUES (?, ?, ?)';
    db.query(query, [department, section, description], (err, result) => {
        ...
        res.status(200).json({ message: 'Section added successfully', id: result.insertId });
    });
});
...
app.put('/sections/:id', (req, res) => {
    const { department, section, description } = req.body;
    const { id } = req.params;
    const query = 'UPDATE sections SET department = ?, section = ?, description = ? WHERE id = ?';
...
app.post('/titles', (req, res) => {
    ...
    const query = 'INSERT INTO titles (department, section, title) VALUES (?, ?, ?)';
...
app.put('/titles/:id', (req, res) => {
    ...
    if (!title || !department || !section) {
        res.status(400).json({ error: 'Title, department, and section are required' });
        return;
    }

    const query = 'UPDATE titles SET title = ?, department = ?, section = ? WHERE id = ?';
...
app.post('/headings', (req, res) => {
    ...
    const query = 'INSERT INTO headings (department, section, title ,heading) VALUES (?, ?, ? ,?)';
    db.execute(query, [department, section, title , heading], (err, results) => {
        ...
        res.status(201).json({ message: 'heading added', titleId: results.insertId });
    });
});
...
app.put('/headings/:id', (req, res) => {
    ...
    if (!title || !department || !section || !heading) {
        res.status(400).json({ error: 'heading,Title, department, and section are required' });
        return;
    }

    const query = 'UPDATE headings SET heading = ?, title = ?, department = ?, section = ? WHERE id = ?';
...
app.get('/api/template/:id', (req, res) => {
  ...
  app.post('/templates', (req, res) => {
    const { title, heading, template, question_type, question_number, questions } = req.body;
    ...
    if (question_type === 'mcq') {
      for (const question of questions) {
        if (!question.options || !Array.isArray(question.options)) {
          return res.status(400).json({ error: 'MCQ questions should have an options array' });
        }
      }
    }
  
    const query = 'INSERT INTO templates (title, heading, template, question_type, question_number, questions) VALUES (?, ?, ?, ?, ?, ?)';
    db.execute(query, [title, heading, template, question_type, question_number, JSON.stringify(questions)], (err, results) => {
      if (err) {
        console.error('Database query error:', err);
        res.status(500).json({ error: 'Database error' });
        return;
      }
      res.status(201).json({ message: 'Template added', templateId: results.insertId });
    });
  });
  ...
  app.put('/templates/:id', (req, res) => {
    ...
    if (!Array.isArray(questions)) {
      return res.status(400).json({ error: 'Invalid format for questions. It should be an array.' });
    }
  
    const query = 'UPDATE templates SET title = ?, heading = ?, template = ?, question_type = ?, question_number = ?, questions = ? WHERE id = ?';
    db.execute(query, [title, heading, template, question_type, question_number, JSON.stringify(questions), id], (err) => {
      if (err) {
        console.error('Database query error:', err);
        res.status(500).json({ error: 'Database error' });
        return;
...
//---------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------------
// Add this endpoint to your existing Express backend code

// In your Express server (Node.js)
