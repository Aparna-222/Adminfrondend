...
function Organization() {
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case 'phone':
        if (!/^\d{10}$/.test(value)) {
          setErrors({ ...errors, [name]: 'Phone number must be a 10-digit number' });
        } else {
          setErrors({ ...errors, [name]: '' });
        }
        break;
      ...
      case 'password':
        ...
        break;
      case 'logo':
        if (!value) { // Check if logo file input is empty
  ...
  const editOrganization = async () => {
    if (validateForm()) {
      ...
      try {
        await axios.put(`${API_URL}/api/organizations/${currentOrgId}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
  ...
  const validateForm = () => {
    const requiredFields = ['name', 'address', 'phone', 'email', 'password', 'person', 'logo'];
    let isValid = true;
    requiredFields.forEach(field => {
      if (field === 'logo') {
        ...
        if (!newOrg[field]?.trim()) { // Use optional chaining to avoid accessing trim on undefined
  ...
  return (
    <div className="organization">
      <div className="organization-header">
        <h2>ORGANIZATIONS</h2>
        {!isAdding && !isEditing && (
          <button className="add-btn" onClick={() => setIsAdding(true)}>Add Organization</button>
        )}
      </div>
      ...
      {(isAdding || isEditing) ? (
        <div className="organization-form">
          ...
          />
          {errors.logo && <span className="error-message" style={{ color: 'red' }}>{errors.logo}</span>}

          <div className="form-buttons">
            <button className="add-btn" onClick={isEditing ? editOrganization : addOrganization}>
          ...
          <tbody>
            {organizations.map((org, index) => (
              <tr key={org.id}>
                ...
                <td><img src={`${API_URL}/${org.logo}`} alt="Logo" style={{ width: '50px', height: '50px' }} /></td>
                <td className="organization-actions">
                  <button className="edit-btn" onClick={() => startEditing(org)}>EDIT</button>
                  <button className="delete-btn" onClick={() => deleteOrganization(org.id)}>DELETE</button>
                </td>
              </tr>
            ))}
          </tbody>
